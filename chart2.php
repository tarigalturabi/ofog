...
<?php
   /* $class = empty($_POST['class'])?$_POST['class']:"";
    ...
    $strXML = "<graph caption='تقرير آداء الطــالب'xAxisName='الفصــل الدراسي'     yAxisName='GPA'      subCaption='بالتقدير'  pieSliceDepth='10' showBorder='1' showNames='1' formatNumberScale='00.00'  decimalPrecision='00.00'  yAxisMinValue='0.0' yAxisMaxValue='4.0' showValues='1'>";
	...
	if ($res) { $i+10;
		...
		while($ors = $database->fetch_array($res)) {
			$strXML .= "<set name='" . $ors['semester'] . "' value='" .GPA( $num,$ors['semester']) . "'  />";
		}
	}
